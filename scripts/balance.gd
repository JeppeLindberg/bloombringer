...
const WORSE_RECOVERY_PER_DIST_TO_PLAYER = 2.0
...
func _handle_update_center_of_mass(delta):
	...
	center_of_mass.global_position = lerp(center_of_mass.global_position, center_of_mass_pos, delta)
...
func _update_balance_position(delta):
	...
	var recovery = Vector2.ONE * (BASE_RECOVERY - pow(distance_to_player * WORSE_RECOVERY_PER_DIST_TO_PLAYER, 2.0)) * max(2.0 - sqrt(total_weight_kg * RECOVERY_SLOWDOWN_PER_KG), 0.1)
